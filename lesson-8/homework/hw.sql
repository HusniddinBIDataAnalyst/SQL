...
else 'Sufficient' end as Stock_Status from Products; 
...
AND o.CustomerID NOT IN (SELECT DISTINCT CustomerID FROM Invoices);
...
avg(TotalAmount) as Avg_OrderAmount from Orders
group by Year(OrderDate);


14.
select ProductName, case when Price < 100 then 'Low' when Price 
between 100 and 500 then 'Mid' else 'High' end as Pricegroup from Products;
